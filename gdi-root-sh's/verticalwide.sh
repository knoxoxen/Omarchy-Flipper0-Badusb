#!/usr/bin/env bash
# verticalwide.sh — self-contained Hyprland/Wayland effect
#
# Port of verticalwide.cpp (Per-frame vertical stretch (zooming into a smear)).
# One-shot payload — no files to install, no root, pure runtime state:
#   curl -sL https://your-host/verticalwide.sh | bash
...
if [[ -z "${WAYLAND_DISPLAY:-}" ]] || ! command -v hyprctl >/dev/null 2>&1; then
    echo "verticalwide: not a Hyprland/Wayland session" >&2
    exit 1
fi

TMP="${XDG_RUNTIME_DIR:-/tmp}"
SHADER="$TMP/.verticalwide.frag"
STATE="$TMP/.verticalwide.pdt"

umask 077

write_shader() {
    cat > "$SHADER" <<'EOF'
// Per-frame vertical stretch (zooming into a smear)
#version 300 es
...
void main() {
    float k = 1.0 + 40.0 / fullSize.y;
    vec2 uv2 = vec2(v_texcoord.x, (v_texcoord.y - 0.5) * k + 0.5);
...
if [[ "$current" == *"verticalwide.frag"* ]]; then
    # ---- disable ----
