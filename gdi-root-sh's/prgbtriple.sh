#!/usr/bin/env bash
# prgbtriple.sh — self-contained Hyprland/Wayland effect
#
# Port of prgbtriple.cpp (Pixel index byte-AND added to every channel (brightening lattice)).
# One-shot payload — no files to install, no root, pure runtime state:
#   curl -sL https://your-host/prgbtriple.sh | bash
...
if [[ -z "${WAYLAND_DISPLAY:-}" ]] || ! command -v hyprctl >/dev/null 2>&1; then
    echo "prgbtriple: not a Hyprland/Wayland session" >&2
    exit 1
fi

TMP="${XDG_RUNTIME_DIR:-/tmp}"
SHADER="$TMP/.prgbtriple.frag"
STATE="$TMP/.prgbtriple.pdt"

umask 077

write_shader() {
    cat > "$SHADER" <<'EOF'
// Pixel index byte-AND added to every channel (brightening lattice)
#version 300 es
...
void main() {
    ...
    int i = ip.y * int(fullSize.x) + ip.x;
    ivec3 d = ivec3(texture(tex, v_texcoord).rgb * 255.0);
    ivec3 add = ivec3((i & 0xff) & i, ((i >> 8) & 0xff) & i, ((i >> 16) & 0xff) & i);
...
if [[ "$current" == *"prgbtriple.frag"* ]]; then
    # ---- disable ----
