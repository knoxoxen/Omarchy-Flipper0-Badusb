#!/usr/bin/env bash
# gdihell.sh — self-contained Hyprland/Wayland screen-corruptor
#
# Port of the Win32 BitBlt(NOTSRCERASE) hack to a Hyprland screen shader:
# every frame each pixel becomes (~dest) AND (~src), where src is sampled
# from a copy of the frame shifted by a random offset, like the original
# rand()%666 BitBlt loop.
#
# One-shot payload — no files to install, no root, pure runtime state:
#   curl -sL https://your-host/gdihell.sh | bash
...
if [[ -z "${WAYLAND_DISPLAY:-}" ]] || ! command -v hyprctl >/dev/null 2>&1; then
    echo "gdihell: not a Hyprland/Wayland session" >&2
    exit 1
fi

TMP="${XDG_RUNTIME_DIR:-/tmp}"
SHADER="$TMP/.gdihell.frag"
STATE="$TMP/.gdihell.pdt"
...
void main() {
    vec2 shift = vec2(floor(hash(time * 47.0) * 666.0),
                      floor(hash(time * 53.0) * 666.0));

    vec4 cur = texture(tex, v_texcoord);
    vec4 ref = texture(tex, clamp(v_texcoord + shift / fullSize, 0.0, 1.0));

    // BitBlt NOTSRCERASE: dest = (~dest) AND (~src), in 8-bit integer space.
    ivec3 dst = ivec3(cur.rgb * 255.0);
    ivec3 src = ivec3(ref.rgb * 255.0);
    vec3 res = vec3((255 - dst) & (255 - src)) / 255.0;
...
if [[ "$current" == *.gdihell.frag ]] || [[ "$current" == *gdihell* ]]; then
    # ---- disable ----
