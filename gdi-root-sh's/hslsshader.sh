#!/usr/bin/env bash
# hslsshader.sh — self-contained Hyprland/Wayland effect
#
# Port of hslsshader.cpp (Every pixel overwritten with a cycling HSB color (rainbow strobe)).
# One-shot payload — no files to install, no root, pure runtime state:
#   curl -sL https://your-host/hslsshader.sh | bash
...
if [[ -z "${WAYLAND_DISPLAY:-}" ]] || ! command -v hyprctl >/dev/null 2>&1; then
    echo "hslsshader: not a Hyprland/Wayland session" >&2
    exit 1
fi

TMP="${XDG_RUNTIME_DIR:-/tmp}"
SHADER="$TMP/.hslsshader.frag"
STATE="$TMP/.hslsshader.pdt"

umask 077

write_shader() {
    cat > "$SHADER" <<'EOF'
// Every pixel overwritten with a cycling HSB color (rainbow strobe)
#version 300 es
...
void main() {
    vec3 res = hsv2rgb(fract(time * 0.06), 1.0, 1.0);
...
if [[ "$current" == *"hslsshader.frag"* ]]; then
    # ---- disable ----
