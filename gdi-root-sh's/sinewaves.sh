#!/usr/bin/env bash
# sinewaves.sh — self-contained Hyprland/Wayland effect
#
# Port of sinewaves.cpp (Per-row horizontal sine-wave distortion).
# One-shot payload — no files to install, no root, pure runtime state:
#   curl -sL https://your-host/sinewaves.sh | bash
...
if [[ -z "${WAYLAND_DISPLAY:-}" ]] || ! command -v hyprctl >/dev/null 2>&1; then
    echo "sinewaves: not a Hyprland/Wayland session" >&2
    exit 1
fi

TMP="${XDG_RUNTIME_DIR:-/tmp}"
SHADER="$TMP/.sinewaves.frag"
STATE="$TMP/.sinewaves.pdt"

umask 077

write_shader() {
    cat > "$SHADER" <<'EOF'
// Per-row horizontal sine-wave distortion
#version 300 es
...
void main() {
    float row = v_texcoord.y * fullSize.y;
    float angle = row * 0.0785398 + time * 20.0;
    float a = sin(angle) * 20.0 / fullSize.x;
    vec3 res = texture(tex, clamp(vec2(v_texcoord.x - a, v_texcoord.y), 0.0, 1.0)).rgb;
...
if [[ "$current" == *"sinewaves.frag"* ]]; then
    # ---- disable ----
