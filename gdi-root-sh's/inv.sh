#!/usr/bin/env bash
# inv.sh — self-contained Hyprland/Wayland effect
#
# Port of inv.cpp (PatBlt PATINVERT: whole screen inverted every tick).
# One-shot payload — no files to install, no root, pure runtime state:
#   curl -sL https://your-host/inv.sh | bash
...
if [[ -z "${WAYLAND_DISPLAY:-}" ]] || ! command -v hyprctl >/dev/null 2>&1; then
    echo "inv: not a Hyprland/Wayland session" >&2
    exit 1
fi
...
write_shader() {
    cat > "$SHADER" <<'EOF'
// PatBlt PATINVERT: whole screen inverted every tick
#version 300 es
...
void main() {
    vec3 res = 1.0 - texture(tex, v_texcoord).rgb;
