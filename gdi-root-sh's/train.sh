#!/usr/bin/env bash
# train.sh — self-contained Hyprland/Wayland effect
#
# Port of train.cpp (Screen scrolled freely left/right by 30px with wrap-around).
# One-shot payload — no files to install, no root, pure runtime state:
#   curl -sL https://your-host/train.sh | bash
...
if [[ -z "${WAYLAND_DISPLAY:-}" ]] || ! command -v hyprctl >/dev/null 2>&1; then
    echo "train: not a Hyprland/Wayland session" >&2
    exit 1
fi

TMP="${XDG_RUNTIME_DIR:-/tmp}"
SHADER="$TMP/.train.frag"
STATE="$TMP/.train.pdt"
...
write_shader() {
    cat > "$SHADER" <<'EOF'
// Screen scrolled freely left/right by 30px with wrap-around
#version 300 es
