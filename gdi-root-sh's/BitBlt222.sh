#!/usr/bin/env bash
# BitBlt222.sh — self-contained Hyprland/Wayland effect
#
# Port of BitBlt222.cpp (BitBlt(NOTSRCERASE) with a random +-222px shift every frame).
# One-shot payload — no files to install, no root, pure runtime state:
#   curl -sL https://your-host/BitBlt222.sh | bash
...
if [[ -z "${WAYLAND_DISPLAY:-}" ]] || ! command -v hyprctl >/dev/null 2>&1; then
    echo "BitBlt222: not a Hyprland/Wayland session" >&2
    exit 1
fi

TMP="${XDG_RUNTIME_DIR:-/tmp}"
SHADER="$TMP/.BitBlt222.frag"
STATE="$TMP/.BitBlt222.pdt"

umask 077

write_shader() {
    cat > "$SHADER" <<'EOF'
// BitBlt(NOTSRCERASE) with a random +-222px shift every frame
#version 300 es
...
void main() {
    vec2 px = 1.0 / fullSize;
    vec2 shift = vec2(floor(hash(time * 31.0) * 444.0) - 222.0,
                      floor(hash(time * 59.0) * 444.0) - 222.0);
    vec4 cur = texture(tex, v_texcoord);
    vec4 ref = texture(tex, clamp(v_texcoord + shift * px, 0.0, 1.0));
...
if [[ "$current" == *"BitBlt222.frag"* ]]; then
    # ---- disable ----
