#!/usr/bin/env bash
# mlt.sh — self-contained Hyprland/Wayland effect
#
# Port of mlt.cpp (SRCCOPY: random 10px columns sluiced down 1px (classic melt)).
# One-shot payload — no files to install, no root, pure runtime state:
#   curl -sL https://your-host/mlt.sh | bash
...
if [[ -z "${WAYLAND_DISPLAY:-}" ]] || ! command -v hyprctl >/dev/null 2>&1; then
    echo "mlt: not a Hyprland/Wayland session" >&2
    exit 1
fi
...
write_shader() {
    cat > "$SHADER" <<'EOF'
// SRCCOPY: random 10px columns sluiced down 1px (classic melt)
#version 300 es
