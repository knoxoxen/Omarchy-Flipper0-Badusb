#!/usr/bin/env bash
# wef.sh — self-contained Hyprland/Wayland effect
#
# Port of wef.cpp (Expanding EllipticRgn NOTSRCCOPY ripple inversions).
# One-shot payload — no files to install, no root, pure runtime state:
#   curl -sL https://your-host/wef.sh | bash
...
if [[ -z "${WAYLAND_DISPLAY:-}" ]] || ! command -v hyprctl >/dev/null 2>&1; then
    echo "wef: not a Hyprland/Wayland session" >&2
    exit 1
fi
...
write_shader() {
    cat > "$SHADER" <<'EOF'
// Expanding EllipticRgn NOTSRCCOPY ripple inversions
#version 300 es
...
void main() {
    ...
    vec2 c = vec2(hash(floor(time * 0.4)) * fullSize.x,
                  hash(floor(time * 0.4) + 1.0) * fullSize.y);
    float r = mod(time * 300.0, 1100.0);
    float on = step(distance(pt, c), r);
    vec3 base = texture(tex, v_texcoord).rgb;
    fragColor = vec4(mix(base, 1.0 - base, on), 1.0);
}
