#!/usr/bin/env bash
# cubes.sh — self-contained Hyprland/Wayland effect
#
# Port of cubes.cpp (Two StretchBlts per frame (zoom in then out) pulsing the whole screen).
# One-shot payload — no files to install, no root, pure runtime state:
#   curl -sL https://your-host/cubes.sh | bash
...
if [[ -z "${WAYLAND_DISPLAY:-}" ]] || ! command -v hyprctl >/dev/null 2>&1; then
    echo "cubes: not a Hyprland/Wayland session" >&2
    exit 1
fi

TMP="${XDG_RUNTIME_DIR:-/tmp}"
SHADER="$TMP/.cubes.frag"
STATE="$TMP/.cubes.pdt"
...
write_shader() {
    cat > "$SHADER" <<'EOF'
// Two StretchBlts per frame (zoom in then out) pulsing the whole screen
#version 300 es
...
void main() {
    float k = 1.0 + 0.02 * sin(time * 5.0);
    vec2 uv2 = (v_texcoord - 0.5) * k + 0.5;
