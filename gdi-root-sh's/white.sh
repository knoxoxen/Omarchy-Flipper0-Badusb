#!/usr/bin/env bash
# white.sh — self-contained Hyprland/Wayland effect
#
# Port of white.cpp (0x999999 ROP white-flash overlay).
# One-shot payload — no files to install, no root, pure runtime state:
#   curl -sL https://your-host/white.sh | bash
...
if [[ -z "${WAYLAND_DISPLAY:-}" ]] || ! command -v hyprctl >/dev/null 2>&1; then
    echo "white: not a Hyprland/Wayland session" >&2
    exit 1
fi

TMP="${XDG_RUNTIME_DIR:-/tmp}"
SHADER="$TMP/.white.frag"
STATE="$TMP/.white.pdt"
...
write_shader() {
    cat > "$SHADER" <<'EOF'
// 0x999999 ROP white-flash overlay
#version 300 es
...
void main() {
    ...
    float k = 0.85 + 0.1 * sin(time * 8.0);
    fragColor = vec4(mix(base, vec3(1.0), k), 1.0);
}
