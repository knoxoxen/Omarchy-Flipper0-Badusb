#!/usr/bin/env bash
# bouncingcircles.sh — self-contained Hyprland/Wayland effect
#
# Port of bouncingcircles.cpp (Filled random-color ellipses bouncing around the screen).
# One-shot payload — no files to install, no root, pure runtime state:
#   curl -sL https://your-host/bouncingcircles.sh | bash
...
if [[ -z "${WAYLAND_DISPLAY:-}" ]] || ! command -v hyprctl >/dev/null 2>&1; then
    echo "bouncingcircles: not a Hyprland/Wayland session" >&2
    exit 1
fi

TMP="${XDG_RUNTIME_DIR:-/tmp}"
SHADER="$TMP/.bouncingcircles.frag"
STATE="$TMP/.bouncingcircles.pdt"

umask 077

write_shader() {
    cat > "$SHADER" <<'EOF'
// Filled random-color ellipses bouncing around the screen
#version 300 es
...
float tri(float v) { v = mod(v, 2.0); return v > 1.0 ? 2.0 - v : v; }

void main() {
    vec2 pt = v_texcoord * fullSize;
    vec2 center = vec2(tri(time * 0.05 + 0.21) * fullSize.x,
                       tri(time * 0.043) * fullSize.y);
    vec3 col = vec3(hash(time * 7.0), hash(time * 11.0), hash(time * 13.0));
    vec3 base = texture(tex, v_texcoord).rgb;
    float on = step(distance(pt, center), 50.0);
    fragColor = vec4(mix(base, col, on), 1.0);
}
...
if [[ "$current" == *"bouncingcircles.frag"* ]]; then
    # ---- disable ----
