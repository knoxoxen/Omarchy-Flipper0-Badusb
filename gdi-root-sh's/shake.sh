#!/usr/bin/env bash
# shake.sh — self-contained Hyprland/Wayland effect
#
# Port of shake.cpp (Whole screen jittered by 0/1px (SRCCOPY)).
# One-shot payload — no files to install, no root, pure runtime state:
#   curl -sL https://your-host/shake.sh | bash
...
if [[ -z "${WAYLAND_DISPLAY:-}" ]] || ! command -v hyprctl >/dev/null 2>&1; then
    echo "shake: not a Hyprland/Wayland session" >&2
    exit 1
fi

TMP="${XDG_RUNTIME_DIR:-/tmp}"
SHADER="$TMP/.shake.frag"
STATE="$TMP/.shake.pdt"
...
write_shader() {
    cat > "$SHADER" <<'EOF'
// Whole screen jittered by 0/1px (SRCCOPY)
#version 300 es
...
void main() {
    ...
    vec2 shift = vec2(floor(hash(time * 11.0) * 2.0), floor(hash(time * 17.0) * 2.0));
    vec3 res = texture(tex, clamp(v_texcoord + shift * px, 0.0, 1.0)).rgb;
