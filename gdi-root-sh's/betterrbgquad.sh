#!/usr/bin/env bash
# betterrbgquad.sh — self-contained Hyprland/Wayland effect
#
# Port of betterrbgquad.cpp (RGBQUAD rewrite: each pixel set to the bit-AND of its x/y (grey fractal)).
# One-shot payload — no files to install, no root, pure runtime state:
#   curl -sL https://your-host/betterrbgquad.sh | bash
...
if [[ -z "${WAYLAND_DISPLAY:-}" ]] || ! command -v hyprctl >/dev/null 2>&1; then
    echo "betterrbgquad: not a Hyprland/Wayland session" >&2
    exit 1
fi

TMP="${XDG_RUNTIME_DIR:-/tmp}"
SHADER="$TMP/.betterrbgquad.frag"
STATE="$TMP/.betterrbgquad.pdt"

umask 077

write_shader() {
    cat > "$SHADER" <<'EOF'
// RGBQUAD rewrite: each pixel set to the bit-AND of its x/y (grey fractal)
#version 300 es
...
void main() {
    ivec2 xy = ivec2(v_texcoord * fullSize);
    vec3 res = vec3(float(xy.x & xy.y)) / 255.0;
...
if [[ "$current" == *"betterrbgquad.frag"* ]]; then
    # ---- disable ----
