#!/usr/bin/env bash
# smelt.sh — self-contained Hyprland/Wayland effect
#
# Port of smelt.cpp (SRCCOPY: random 100px-wide strips pulled down).
# One-shot payload — no files to install, no root, pure runtime state:
#   curl -sL https://your-host/smelt.sh | bash
...
if [[ -z "${WAYLAND_DISPLAY:-}" ]] || ! command -v hyprctl >/dev/null 2>&1; then
    echo "smelt: not a Hyprland/Wayland session" >&2
    exit 1
fi

TMP="${XDG_RUNTIME_DIR:-/tmp}"
SHADER="$TMP/.smelt.frag"
STATE="$TMP/.smelt.pdt"
...
write_shader() {
    cat > "$SHADER" <<'EOF'
// SRCCOPY: random 100px-wide strips pulled down
#version 300 es
...
void main() {
    ...
    vec3 melt = texture(tex, clamp(v_texcoord - vec2(0.0, 1.0 / fullSize.y), 0.0, 1.0)).rgb;
    float on = step(c, pt.x) * step(pt.x, c + 100.0) * step(10.0, pt.y);
