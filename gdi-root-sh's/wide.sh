#!/usr/bin/env bash
# wide.sh — self-contained Hyprland/Wayland effect
#
# Port of wide.cpp (Per-frame horizontal stretch (zooming into a smear)).
# One-shot payload — no files to install, no root, pure runtime state:
#   curl -sL https://your-host/wide.sh | bash
...
if [[ -z "${WAYLAND_DISPLAY:-}" ]] || ! command -v hyprctl >/dev/null 2>&1; then
    echo "wide: not a Hyprland/Wayland session" >&2
    exit 1
fi

TMP="${XDG_RUNTIME_DIR:-/tmp}"
SHADER="$TMP/.wide.frag"
STATE="$TMP/.wide.pdt"
...
write_shader() {
    cat > "$SHADER" <<'EOF'
// Per-frame horizontal stretch (zooming into a smear)
#version 300 es
...
void main() {
    float k = 1.0 + 40.0 / fullSize.x;
    vec2 uv2 = vec2((v_texcoord.x - 0.5) * k + 0.5, v_texcoord.y);
