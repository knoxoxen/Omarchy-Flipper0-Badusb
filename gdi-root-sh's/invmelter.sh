#!/usr/bin/env bash
# invmelter.sh — self-contained Hyprland/Wayland effect
#
# Port of invmelter.cpp (NOTSRCCOPY: random 10px columns sluiced down 1px, inverted).
# One-shot payload — no files to install, no root, pure runtime state:
#   curl -sL https://your-host/invmelter.sh | bash
...
if [[ -z "${WAYLAND_DISPLAY:-}" ]] || ! command -v hyprctl >/dev/null 2>&1; then
    echo "invmelter: not a Hyprland/Wayland session" >&2
    exit 1
fi

TMP="${XDG_RUNTIME_DIR:-/tmp}"
SHADER="$TMP/.invmelter.frag"
STATE="$TMP/.invmelter.pdt"

umask 077

write_shader() {
    cat > "$SHADER" <<'EOF'
// NOTSRCCOPY: random 10px columns sluiced down 1px, inverted
#version 300 es
...
void main() {
    ...
    float c = floor(hash(floor(time * 30.0)) * fullSize.x);
    vec3 base = texture(tex, v_texcoord).rgb;
    vec3 melt = 1.0 - texture(tex, clamp(v_texcoord - vec2(0.0, 1.0 / fullSize.y), 0.0, 1.0)).rgb;
    float on = step(c, pt.x) * step(pt.x, c + 10.0) * step(1.0, pt.y);
    fragColor = vec4(mix(base, melt, on), 1.0);
}
...
if [[ "$current" == *"invmelter.frag"* ]]; then
    # ---- disable ----
