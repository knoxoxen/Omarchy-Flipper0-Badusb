#!/usr/bin/env bash
# mythlasathome.sh — self-contained Hyprland/Wayland effect
#
# Port of mythlasathome.cpp (PlgBlt parallelogram sheet-shear of the whole screen, repainted).
# One-shot payload — no files to install, no root, pure runtime state:
#   curl -sL https://your-host/mythlasathome.sh | bash
...
if [[ -z "${WAYLAND_DISPLAY:-}" ]] || ! command -v hyprctl >/dev/null 2>&1; then
    echo "mythlasathome: not a Hyprland/Wayland session" >&2
    exit 1
fi

TMP="${XDG_RUNTIME_DIR:-/tmp}"
SHADER="$TMP/.mythlasathome.frag"
STATE="$TMP/.mythlasathome.pdt"

umask 077

write_shader() {
    cat > "$SHADER" <<'EOF'
// PlgBlt parallelogram sheet-shear of the whole screen, repainted
#version 300 es
...
void main() {
    float inc = floor(hash(time * 4.0) * 700.0);
    if (hash(time * 7.0) < 0.5) inc = -inc;
    float k = 2.0 * inc / fullSize.x;
    vec2 uv2 = vec2(v_texcoord.x + (v_texcoord.y - 0.5) * k, v_texcoord.y);
    vec3 res = texture(tex, clamp(uv2, 0.0, 1.0)).rgb;
...
if [[ "$current" == *"mythlasathome.frag"* ]]; then
    # ---- disable ----
